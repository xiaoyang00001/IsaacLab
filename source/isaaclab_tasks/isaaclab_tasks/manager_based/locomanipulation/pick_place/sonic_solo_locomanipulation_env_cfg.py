...
class SonicSoloLocomanipulationEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # XR 锚点：仿照 locomanipulation_g1_env_cfg.py 里 Robot/RemoteRobot 双机的
        # pelvis 锚定方式，把 OpenXR 房间原点对齐到 sonic_robot 脚下（anchor_pos
        # Z=-0.82 把锚点从 pelvis 高度下沉到落地点，用户真实站立时的头部高度就
        # 自然落在机器人大致的视线高度，而不是锁死一个刚性头部摄像机）。
