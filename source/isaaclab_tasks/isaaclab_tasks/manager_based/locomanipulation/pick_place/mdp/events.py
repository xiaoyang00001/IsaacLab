...
def align_walker_robot_to_conveyor(
    ...
    walker_x_offset: 相对 robot1 的额外 X 偏移（sonic 用 3.0 与 walker 错开同框观察）。
    """
    if env_ids is None:
        env_ids = torch.arange(env.scene.num_envs, device=env.device, dtype=torch.long)
    ...
    for i, env_id in enumerate(env_ids.tolist()):
        ...
        if conveyor_prim is None or not conveyor_prim.IsValid():
            _diag_print(
                f"[locomanip_event] walker align: conveyor '{conveyor_prim_name}' not found for env_{env_id}"
            )
            continue
        ...
        walker_state[i, 1] = robot1_y - walker_y_behind
        # Z 保持 default_root_state 中的高度
        ...
        if i == 0:
            _diag_print(
                f"[locomanip_event] {walker_robot_name} placed at "
...
# ======================================================================
# conveyor-loop-totes 流水线事件（合并自 feat/conveyor-loop-totes）：
#   背景刚体 kinematic 锁定 + 流水线送筐驱动。函数名与上方 SONIC 事件不冲突。
# ======================================================================
