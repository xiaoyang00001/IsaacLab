...
class G1LowerBodyStandingHeadPoseRetargeter(RetargeterBase):
    ...
    def retarget(self, data: dict) -> torch.Tensor:
        ...
        # OpenXR poses are typically ordered as [x, y, z], where y is up.
        # Locomotion should use the ground plane (x/z), not the vertical axis (y).
        head_xy = head_pose[[0, 2]]
        ...
        if np.linalg.norm(delta_xy_world) > self.cfg.recenter_distance:
            self._set_reference(head_xy, head_yaw)
            self._filtered_command = torch.tensor(
        ...
        delta_xy_local = np.clip(
            delta_xy_local,
            -self.cfg.max_position_offset,
            self.cfg.max_position_offset,
        )

        vx = self._apply_deadzone(delta_xy_local[0], self.cfg.position_deadzone) * self.cfg.position_scale
        vy = self._apply_deadzone(delta_xy_local[1], self.cfg.position_deadzone) * self.cfg.position_scale
        ...
        if np.linalg.norm(delta_xy_world) > self.cfg.comfort_position_radius:
            self._reference_xy = self._reference_xy + self.cfg.reference_follow_rate * delta_xy_world
