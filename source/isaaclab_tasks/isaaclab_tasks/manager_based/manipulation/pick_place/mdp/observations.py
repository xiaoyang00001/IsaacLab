...
def get_eef_pos(env: ManagerBasedRLEnv, link_name: str, robot_asset_name: str = "robot") -> torch.Tensor:
    robot = env.scene[robot_asset_name]
    body_pos_w = robot.data.body_pos_w
    left_eef_idx = robot.data.body_names.index(link_name)
    left_eef_pos = body_pos_w[:, left_eef_idx] - env.scene.env_origins

    return left_eef_pos


def get_eef_quat(env: ManagerBasedRLEnv, link_name: str, robot_asset_name: str = "robot") -> torch.Tensor:
    robot = env.scene[robot_asset_name]
...
def get_robot_joint_state(
    ...
    robot = env.scene[robot_asset_name]
    indexes, _ = robot.find_joints(joint_names)
    indexes = torch.tensor(indexes, dtype=torch.long)
    robot_joint_states = robot.data.joint_pos[:, indexes]
...
def get_all_robot_link_state(
    ...
    body_pos_w = env.scene[robot_asset_name].data.body_link_state_w[:, :, :]
