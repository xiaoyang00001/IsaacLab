...
1. **身体姿态镜像**（来自远端 UDP）：`mirror_joint_names` 覆盖髋/膝/踝/腰/肩/肘/腕关节，根节点位姿通过独立的 `g1_root` 流镜像（`root_motion_mode="source"`，不启用支撑脚兜底估计）。镜像关节按 `pd_drive_joint_names`（默认匹配肩/肘/腕）分成两组写入：
   ...
   - **PD 弹簧驱动组**（手臂，默认启用）：只下发 `set_joint_position_target` / `set_joint_velocity_target`，不硬写关节状态。手臂由隐式执行器（`stiffness=500, damping=30`，见下文「物理抓取的四个前提」）按力矩拉向目标——碰到箱子推不动时，跟踪误差转化为持续夹持力，静摩擦得以建立，**双臂抱箱搬运因此可行**。遥操作者在远端 MuJoCo 侧没有箱子阻挡，镜像目标会自然落到箱子"内部"，等价于自动过冲。把 `pd_drive_joint_names` 设为空列表可回退到旧的全运动学硬写模式（手臂变成"瞬移墙"，会把箱子挤出且抱不住）。
...
| `write_joint_state` | `False` | 手指只走 PD 位置目标，不再硬写关节状态（ec1732ddc 曾设 True 解决「看起来没完全闭合」，但硬写会每步覆盖接触解算导致手指穿进物体；PD 下手指顶住物体表面即停，「不完全闭合」是正确物理行为） |

### 物理抓取的四个前提（2026-07-09 排查穿模/弹飞问题的结论）

抱箱/捏取要产生真实且稳定的接触力，四件事缺一不可：

1. **接触面要有碰撞体**：原版 `g1_43dof.usd` 的 `left/right_elbow_link`（前臂主体）、`wrist_roll_link`、`wrist_yaw_link` **完全没有碰撞体**，双臂环抱时前臂直接穿过箱子。已用 [scripts/gr00t_wbc/add_g1_forearm_colliders.py](../scripts/gr00t_wbc/add_g1_forearm_colliders.py) 按视觉网格尺寸补上 6 个碰撞胶囊（沿用原资产 `collisions` prim 的 purpose/visibility 约定），原文件备份为 `g1_43dof.usd.bak_no_forearm_collision_20260709`。**每台加载该 USD 的机器都要跑一次这个脚本**（需要 `pip install usd-core`）。
2. **手臂关节走 PD 不硬写**：`pd_drive_joint_names`（见上文第 1 点），跟踪误差转化为夹持力。
3. **手指关节走 PD 不硬写**：`write_joint_state=False`（本表），否则手指接触每步被硬写覆盖、穿进箱子。
4. **力矩上限封顶在真实电机量级**：`effort_limit_sim` 肩/肘 25、腕 5、指节 3 N·m（`G1_43DOF_GR00T_CFG` 的 `arms`/`hands` 执行器）。模板值（手臂 300、指节 60 N·m）会把镜像过冲放大成上千牛的夹持力——遥操作者在远端合拢手臂时没有箱子阻挡，过冲远大于受控实验的 0.25 rad——箱子接触瞬间像捏西瓜籽一样被弹飞（「一抱就飞」）。**刚度/阻尼必须与力矩上限匹配**：3000 配 25 N·m 时 PD 线性区仅 25/3000 ≈ 8 mrad，误差稍大即力矩饱和、阻尼失效，手臂自激振荡（上下摆动，MuJoCo 侧静止也摆）。现值 stiffness 500 / damping 30：线性区 ≈ 3°，阻尼比 ≈ 2（过阻尼），重力静差 ~0.01 rad 不可察觉；夹持力仍由 effort 封顶（每臂约 80 N、指尖约 60 N），与刚度无关。
