...
这也是 B 键 yaw recenter 仍需要 `--teleop_device handtracking` 的原因：B 键绑定在
`OpenXRDevice.__init__` 里通过 XRCore event generator 注册；它需要
`OpenXRDevice` 被构造，但不需要 `_get_raw_data()` 进入
...
-Xr 开关
  ...
  -> right B release 触发 XRCore button event，调用 recenter_yaw_to_anchor_prim()
```
...
- `晓阳全身001` 使用 key `"motion_controllers"`，并挂
  ...
  recenter。
- 当前 SONIC Solo/Fullscene 只需要 XR 锚点和 B 键 recenter，不做手柄夹爪遥操，
  因此 key 仍用 `"handtracking"`，且不配置 retargeter。B 键事件由 XRCore 直接触发，
  不依赖 `MOTION_CONTROLLER` 数据流。
