...
| 2. `--teleop_device` 含 `handtracking` | 同上（teleop_se3_agent.py:75-77 自动置 xr=True） | 当前 `-Xr` 仍会显式追加，用于命中 `env_cfg.teleop_devices["handtracking"]` 并构造 `OpenXRDevice` |
...
| `J` | **手动摔倒恢复**：原地扶正——保留当前 XY+yaw，root 回默认高度、roll/pitch 归零，关节回默认站姿、速度清零，然后重新锁根 + 重走 settle | `SonicDeployTargetAction.recover_standing`（回调延迟到主循环执行，与 R 同模式） |
...
> `reset_scene_to_default`（mode="reset"），并新增 `J` 原地恢复 + 自动摔倒恢复。

> 按键选择坑（2026-07-06）：恢复键最初绑 `H`，实测与 Isaac Sim Edit 菜单的
> **Toggle Visibility 热键冲突**（`isaacsim.gui.menu` edit_menu.py，无修饰键 `H`，
> 选中 prim 时生效）——按 H 机器人站起来的同时选中的 prim（如地面）被隐藏，
> 只剩 DomeLight，**场景变全白**。已换 `J`（紧邻 H、无任何 Kit 绑定）。误按 H
> 隐藏了东西用 Edit → Unhide All（或 Stage 面板点回该 prim 的眼睛图标）恢复。
> Kit 无修饰热键占用清单（本机 Isaac Sim 5.1 实查）：SPACE 播放/暂停、
> Q/W/E/R/T 工具、F 聚焦、H 可见性、P 设父级、ESC/UP/DEL/F2/F10；
> Se3Keyboard 内部另占 L/K/W/S/A/D/Q/E/Z/X/T/G/C/V。可选剩余安全键：
> B（文档语境易与 XR 手柄 B 混淆，弃）、I/M/N/O/Y。
...
| yaw recenter | 不需要 | 启动自动一次；B 键仅在 `SONIC_XR_ENABLE_B_RECENTER=1` 时启用 |
...
- `xr_cfg.py` / `xr_anchor_utils.py` / `openxr_device.py`：2026-07-06 补齐
  `晓阳全身001` 的 head/pelvis 分离、轴向 yaw 计算和 recenter。该项用于
  修正进入 XR 后头显视觉前向与机器人头部/身体前向固定差 90 度的问题；当前默认
  启动自动 recenter，B 键手动 recenter 仅在 `SONIC_XR_ENABLE_B_RECENTER=1` 时绑定。
- 该分支还有手柄遥操输入侧改动（`G1GripperMotionControllerRetargeter`、
  ...
  右 A 键默认 `toggle_anchor_rotation`，默认不再绑定右 B。
...
（`teleop_se3_agent.py` deploy_target_mode 分支的设备选择逻辑）。

这也是 XR 锚点同步仍需要 `--teleop_device handtracking` 的原因：锚点同步和可选
B 键绑定都在 `OpenXRDevice.__init__` 里注册；它需要 `OpenXRDevice` 被构造，
但不需要 `_get_raw_data()` 进入
`RetargeterBase.Requirement.MOTION_CONTROLLER` 分支，也不要求配置
`motion_controllers` retargeter。
...
-Xr 开关
  ...
  -> 第一帧头显 pose 可用后自动调用 recenter_yaw_to_anchor_prim()
  -> 若 SONIC_XR_ENABLE_B_RECENTER=1，再额外绑定 right B release 手动 recenter
```
...
- `晓阳全身001` 使用 key `"motion_controllers"`，并挂
  `G1GripperMotionControllerRetargeterCfg(use_right_b_button=False)`；该 retargeter
  的 `get_requirements()` 返回 `MOTION_CONTROLLER`，所以会轮询控制器 pose/input，
  用 trigger/grip/A 等控制手指。`use_right_b_button=False` 是为了把右 B 留给 yaw
  recenter。
- 当前 SONIC Solo/Fullscene 只需要 XR 锚点同步和启动自动 recenter，不做手柄夹爪遥操，
  因此 key 仍用 `"handtracking"`，且不配置 retargeter。可选 B 键事件由 XRCore
  直接触发，不依赖 `MOTION_CONTROLLER` 数据流。

`sonic_robot` 在两个场景里都是同一个 `SONIC_G1_29DOF_CFG`，prim path 固定为
`{ENV_REGEX_NS}/SONICRobot`（源码定位：`locomanipulation_g1_env_cfg.py:491`），
带 `pelvis` link。`-XrView third` 仍使用 2026-07-02 的 pelvis 方案：
`anchor_pos` Z 偏移 -0.82，把锚点从 pelvis 高度下沉到落地点附近，用户真实站立时
的头部高度会自然落在机器人大致的视线高度；旋转模式沿用默认 FIXED，不随机器人
转身而转动房间朝向，避免眩晕。默认 `-XrView first` 则使用上方 head/pelvis 分离
方案，位置跟 `torso_link/head_link`，yaw 跟 `pelvis`。
...
- `sonic_solo_locomanipulation_env_cfg.py`、`sonic_fullscene_locomanipulation_env_cfg.py`
  新增 import（`DevicesCfg`、`OpenXRDeviceCfg`、`XrCfg`）+ `build_sonic_xr_cfg()`
  / `__post_init__` 锚点配置
