...
### 阶段 A 落地：SONIC actuator 对齐（2026-05-25，分支 `gr00t-sonic-actuator-match`）

切到新分支 `gr00t-sonic-actuator-match` 实施 A 修复。

**变更**：[locomanipulation_g1_env_cfg.py:84-227](../source/isaaclab_tasks/isaaclab_tasks/manager_based/locomanipulation/pick_place/locomanipulation_g1_env_cfg.py#L84-L227) `SONIC_G1_29DOF_CFG.actuators` 整体替换：
- IsaacLab DCMotor → `ImplicitActuatorCfg`（5 个 group: legs / feet / waist / waist_yaw / arms）
- NATURAL_FREQ = 10×2π、DAMPING_RATIO = 2.0
- ARMATURE: 5020=0.0036、7520_14=0.0102、7520_22=0.0251、4010=0.00425
- STIFFNESS = armature × NATURAL_FREQ²，DAMPING = 2 × DAMPING_RATIO × armature × NATURAL_FREQ
- feet / waist 用 2× 基础值
- 不动 G1_29DOF_CFG，robot/walker_robot/remote_robot 仍用 DCMotor

**实测对比**（headless smoke + GUI 实测）：

| step | DCMotor (旧) | SONIC actuator (新) | 变化 |
|---|---|---|---|
| 1 | 2.56 | 2.56 | reset 同步主导 |
| 3 | 5.93 | **3.44** | **-42%** ✅ |
| 5 | 10.10 | **5.32** | **-47%** ✅ |
| 10 | 11.25 | 12.80 | ≈ 持平 |
| 20 | 17.67 | **11.67** | **-34%** ✅ |
| 50 | 16.49 | 14.48 | -12% |
| 100 | — | 10.53 | — |
| 150 | — | 14.99 / joint_pos 3.09 撞限位 | — |
| 200 | — | 16.73 / joint_pos 2.62 | — |

**GUI 实测**：用户报告"早期腿部是有动作，但是立马就抽搐了" — 与数值演化吻合（step 3-5 raw 减半 → 早期腿动 ✅；step 10+ 仍发散 → 抽搐 ❌）。

### 阶段 A 后续：obs 字段全面查证（2026-05-25）

逐字段对比训练 `sonic_release/config.yaml.observations.policy` vs 我们 `_push_history`：

| 字段 | 训练 func | 我们 | 状态 |
|---|---|---|---|
| `base_ang_vel` | IsaacLab `base_ang_vel` → `root_ang_vel_b` | `root_ang_vel_b` | ✅ |
| `joint_pos` | IsaacLab `joint_pos_rel` → `joint_pos - default_joint_pos` | `joint_pos - default_joint_pos` | ✅ |
| `joint_vel` | IsaacLab `joint_vel_rel` → `joint_vel - default_joint_vel` | `joint_vel`（绝对） | ✅ 实质一致（G1 default_joint_vel = 0） |
| `actions` | IsaacLab `last_action` (raw) | `_last_action` | ✅ |
| `gravity_dir` | groot custom：`quat_inv(robot_anchor_quat_w).apply(down_dir=[0,0,-1])` | `projected_gravity_b` | ✅ IsaacLab `GRAVITY_VEC_W` 归一化（不是 g=9.81），与 down_dir 同量级 |

**obs 字段全部对齐**。已查证对齐总结：
- ✅ actuator (本轮 A)
- ✅ sim_dt + decimation (1/200 + 4 = 50Hz control)
- ✅ obs 字段单位 + 坐标系
- ✅ joint mapping (SONIC[i] → USD joint name)
- ✅ reset 同步 robot 到 mocap[0]
- ✅ 时间窗 (dt_future=0.1s, mocap advance=0.6 frame/step)
- ✅ FK body_pos in pelvis local frame (F4)

### 阶段 A 剩余可疑根因（缩窄到一项）

唯一未对齐：**mocap PKL 是 30fps 原始数据，未经 motion_lib `target_fps=50` 重采样**。

训练时 motion_lib 用 SLERP 把 30fps mocap → upscale 到 50fps（1202 帧 → 2003 帧），future ref 取的是真实 50Hz 离散点。我们用原 30fps 离散点，时间上对齐但**采样点不一样**：
- 训练: mocap-50fps[0, 5, 10, ..., 45] (10 帧, 100ms 间隔, 跨度 0.9s)
- 我们: mocap-30fps[0, 3, 6, ..., 27] (10 帧, ~100ms 间隔, 跨度 0.9s)

时间对，但中间 ~67% 的帧是不同的离散位置 → SONIC 看到的 motion 轨迹精度比训练低，第 10 帧累积误差大 → 第二段反馈循环。

**下一步**：mocap 50Hz SLERP 重采样（30 分钟工作量）：
- [precompute_mocap_body_pos.py](../scripts/tools/precompute_mocap_body_pos.py) 加 50Hz 重采样输出 .npy
- `_load_mocap` 加 50Hz 重采样 dof + root_rot
- `_mocap_step` 调回 5、`_mocap_advance_per_step` 调到 1.0
- 验证是否消除 step 10+ 反馈循环

### 阶段 A 后续 2 落地：mocap 50Hz SLERP 重采样（2026-05-25）

**实施**：
1. [precompute_mocap_body_pos.py](../scripts/tools/precompute_mocap_body_pos.py) 调 `fk_batch(interpolate_data=True, target_fps=50)`，让 Humanoid_Batch.interploate_pose 做 SLERP+线性重采样，输出 (2002, 14, 3) 50fps .npy
2. [`_load_mocap`](../source/isaaclab_tasks/isaaclab_tasks/manager_based/locomanipulation/pick_place/mdp/actions.py) 用 scipy.spatial.transform.Slerp 把 root_rot 重采样到 50fps，dof + root_trans 用 np.interp 线性插值，帧数与 .npy 对齐（2002）
3. `_mocap_step = 5`、`_mocap_advance_per_step = 1.0`（mocap_fps=50 / sim_fps=50）

**实测对比**（headless smoke 阶段 A actuator + 50fps）：

| step | actuator + 30fps | actuator + 50fps | 变化 |
|---|---|---|---|
| 50 | 14.48 | 14.48 | 0% |
| 100 | 10.53 | 10.00 | -5% |
| 150 | 14.99 | 17.62 | +18% |
...
**结论：50fps 重采样数值上几乎无差异**。

**原因解释**：30fps + `_mocap_step=3` 和 50fps + `_mocap_step=5` 取的 future ref 时间点本来就近似（都是 0/100/200/.../900ms），重采样只增加"中间帧"但 SONIC 不看中间帧；中间帧只影响 advance per step 的"颗粒度"（30fps 时 0.6 frame/step 累积 vs 50fps 时整 1 frame/step），但 obs 序列几乎一致。

**保留 50fps 决定**：即使数值无改善，与训练 spec (`target_fps=50`) 严格对齐才是正确实现，避免未来 mocap 数据替换或扩展时遗留隐患。

### 阶段 A 总结：所有外部对齐已穷尽，反馈循环仍未消除

A 路径累计对齐 7 项：actuator / sim_dt / obs 字段 / joint mapping / reset 同步 / 时间窗 / FK body_pos / mocap 50fps。除 actuator 在 step 1-5 有 -42%~-47% 明显改善外，其余项都是"对齐 spec 但闭环数值无实质改善"。

**结论：反馈循环根因不在 A 路径范围内**。剩余真正可能的根因：
- **B. PyTorch ckpt vs ONNX 推理差异**：ONNX 模型可能缺少 obs normalization 等预处理层
- **encoder mode_id 选错**：g1 mode (encoder_index=0) 可能不是 sample mocap 适配的（teleop=1 / smpl=2 训练时数据分布不同）
- **训练时 obs noise vs 推理时 obs 无 noise**：训练 obs term 有 AdditiveUniformNoise，可能 SONIC 学到了"在 noise 中识别 robot 状态"，推理时无 noise 反而 OOD
- **ckpt 与 task 不匹配**：sonic_release 可能是 BeyondMimic / whole_body_tracking 的特定任务训练，不是 walking task

这些都需要走 B 路径（接 motion_lib + gear_sonic[training] + PyTorch ckpt）才能进一步验证。本分支 A 路径正式收尾。

### 阶段 A 后续 TODO（不在本分支执行）

- **B（接 motion_lib + PyTorch ckpt + obs normalization 验证）**：3-4 小时，最有可能解决残余反馈循环
- **action_scale 探针**：测了 scale=0.05（从 1.0 降 20 倍）—— raw action 仍爆炸到 17.6（step50），scale 不影响 ONNX 输出，根因在 decoder history 累积（`_last_action` 写入 decoder 导致误差放大）
- **B 路径唯一出路**：接入 gear_sonic[training] + PyTorch ckpt，完整复现训练时的 history 初始化和 noise 策略
