...
from isaaclab.devices.openxr.retargeters.humanoid.unitree.g1_head_pose_locomotion import (
    G1LowerBodyStandingHeadPoseRetargeterCfg,
)
...
MOBILE_G1_29DOF_CFG = G1_29DOF_CFG.copy()
MOBILE_G1_29DOF_CFG.spawn.articulation_props.fix_root_link = False
MOBILE_G1_29DOF_CFG.spawn.rigid_props.disable_gravity = False
...
MOBILE_G1_29DOF_CFG.init_state.pos = (0.0, 0.0, 0.75)
MOBILE_G1_29DOF_CFG.init_state.rot = (1, 0, 0, 0)
...
class LocomanipulationG1SceneCfg(InteractiveSceneCfg):
    ...
    # 测试箱子：落�?ConveyorBelt_A08_06 传送带 +y 端（入料端）�?
    # 传送带�?Y 轴延伸，y∈[-11.42, -8.70]，中�?x=15.30，带�?z=-0.0155�?
    # 箱子�?-y 方向流向机器人（robot1 y�?12.50，robot2 y�?13.07）�?
    ...
    robot: ArticulationCfg = MOBILE_G1_29DOF_CFG
...
class ActionsCfg:
    ...
    lower_body_joint_pos = AgileBasedLowerBodyActionCfg(
        asset_name="robot",
        joint_names=LOWER_BODY_JOINT_NAMES,
        obs_group_name="lower_body_policy",
        policy_path="./assets/model/policy1.onnx",
        hip_height=0.72,
        policy_output_scale=0.05,
        action_smoothing=0.10,
        command_scale=0.35,
        enable_policy_when_moving=True,
        root_motion_scale=1.35,
        root_motion_smoothing=0.70,
    )
    ...
    object_sync = ZmqObjectSyncActionCfg(asset_name="test_box", role=ZMQ_SYNC_ROLE, endpoint="tcp://192.168.1.60:15555")
    object_sync1 = ZmqObjectSyncActionCfg(asset_name="test_box1", role=ZMQ_SYNC_ROLE, endpoint="tcp://192.168.1.60:15555")
...
class EventsCfg:
    """Runtime events for the warehouse scene."""


    # prestartup：在 PhysX 初始化前�?SM_CardBoxD_05.usd 注入刚体物理 API�?
    ...
    # 启动时打�?ConveyorBelt_A08_06 的世界包围盒，用于校�?test_box 坐标�?
    ...
    # �?0.05s 强制恢复箱子�?-y 速度，沿传送带流向机器人（y�?12~-13）�?
...
class LocomanipulationG1EnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        print(
            "[IsaacLab] [RobotMapping] "
            f"controllable_asset=robot init_pos={self.scene.robot.init_state.pos} init_rot={self.scene.robot.init_state.rot}; "
            f"passive_asset=remote_robot init_pos={self.scene.remote_robot.init_state.pos} init_rot={self.scene.remote_robot.init_state.rot}"
        )
        ...
        self.actions.upper_body_ik.asset_name = "robot"
        self.actions.upper_body_ik.controller.articulation_name = "robot"
        print(
            "[IsaacLab] [UpperBodyBinding] "
            f"asset_name={self.actions.upper_body_ik.asset_name} "
            f"articulation_name={self.actions.upper_body_ik.controller.articulation_name}"
        )
        ...
        # Keep the XR world fixed to the user's room space. Binding the anchor to the
        # robot pelvis makes the entire scene appear to rotate whenever the robot turns.
        # That breaks head-referenced teleoperation even if the lower-body policy itself
        # is behaving better.
                ...
                "handtracking": OpenXRDeviceCfg(
                    retargeters=[
                        G1LowerBodyStandingHeadPoseRetargeterCfg(
                            ...
                            planar_axis_indices=(0, 1),
                            position_scale=1.0,
                            forward_position_scale=1.0,
                            lateral_position_scale=1.0,
                            yaw_scale=2.0,
                            position_deadzone=0.06,
                            yaw_deadzone=0.12,
                            max_position_offset=1.50,
                            comfort_position_radius=10.0,
                            reference_follow_rate=0.0,
                            recenter_distance=3.50,
                            max_linear_velocity=2.20,
                            max_yaw_rate=3.60,
                            yaw_suppression_linear_speed=2.0,
                            enable_vx=True,
                            enable_vy=True,
                            smoothing_factor=0.35,
                            yaw_smoothing_factor=0.55,
                        ),
                        G1TriHandUpperBodyMotionControllerRetargeterCfg(
