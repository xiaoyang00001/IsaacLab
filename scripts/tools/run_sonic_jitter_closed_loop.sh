...
DEPLOY_BIN="${SONY_REPO}/gear_sonic_deploy/target/release/g1_deploy_onnx_ref"
...
if [[ "$JITTER_INPUT" == "keyboard" ]]; then
    ...
    tmux new-window -t "$SESSION" -n deploy \
        "cd '${SONY_REPO}/gear_sonic_deploy' && export DDS_INTERFACE=lo && source scripts/setup_env.sh && just run g1_deploy_onnx_ref \"\$DDS_INTERFACE\" '$DECODER_MODEL' reference/example --obs-config '$OBS_CONFIG' --encoder-file '$ENCODER_MODEL' --planner-file '$PLANNER_MODEL' --input-type keyboard --output-type all --zmq-out-port 5557 --zmq-out-topic g1_debug --disable-crc-check |& tee '$DEPLOY_LOG'; exec bash" \
        || { log "✗ tmux deploy 窗口启动失败"; exit 1; }
elif [[ "$PROTO" == "1" && "$(readlink -f -- "$POLICY_ROOT")" == "$(readlink -f -- "${SONY_REPO}/gear_sonic_deploy/policy/release")" ]]; then
